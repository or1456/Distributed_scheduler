...
int PRBalloc_to_locationandbandwidth_mwnl(int N_UEs,int NPRB, int RBstart, int BWPsize)
{
  // printf("RBstart: %d \n", RBstart);
  AssertFatal(NPRB>0 && (NPRB + RBstart <= BWPsize),
              "Illegal NPRB/RBstart Configuration (%d,%d) for BWPsize %d\n",
  ...
  if (NPRB <= 1 + (BWPsize >> 1))
    return (N_UEs*BWPsize * (NPRB - 1) + RBstart);
  else
    return (N_UEs*BWPsize * (BWPsize + 1 - NPRB) + (BWPsize - 1 - RBstart));
}
