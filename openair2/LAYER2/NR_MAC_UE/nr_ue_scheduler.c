...
uint8_t nr_locate_BsrIndexByBufferSize(const uint32_t *table, int size, int value)
{
  ...
  while (ju - jl > 1) { //If we are not yet done,
    ...
    LOG_D(NR_MAC, "[UE] searching BSR index %d for (BSR TABLE %d < value %d)\n",
          jm, table[jm], value);
  }
                                    ...
                                    NR_UE_MAC_CE_INFO *mac_ce_p)
{
  ...
  mac_ce_p->bsr_len = mac_ce_p->bsr_ce_len + mac_ce_p->bsr_header_len; // 2 bytes
                                      ...
                                      NR_UE_MAC_CE_INFO *mac_ce_p)
{
  ...
  */
  // printf("mac_ce_p->sdu_length_total:%d \n",mac_ce_p->sdu_length_total);
  if (mac_ce_p->sdu_length_total) { // mac_ce_p->sdu_length_total==0
                      ...
                      uint16_t buflen)
{
  ...
  is placed in the MAC PDU before going on to next high priority logical channel
      */
      ...
      // printf("buflen%d mac_ce_p->total_mac_pdu_header_len%d mac_ce_p->sdu_length_total%d sh_size%d \n",
      //         buflen,mac_ce_p->total_mac_pdu_header_len,mac_ce_p->sdu_length_total,sh_size);
      buflen_remain = buflen - (mac_ce_p->total_mac_pdu_header_len + mac_ce_p->sdu_length_total + sh_size);
