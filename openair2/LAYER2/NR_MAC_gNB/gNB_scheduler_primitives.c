...
// mwnl or
// variable area.
static uint16_t first_rnti = 0;
static uint16_t second_rnti = 0;
static uint8_t DSI_flag = 0;

static uint8_t temp_buffer_size = 0;
static uint8_t temp_ndi = 0;
static uint8_t temp_rv = 0;
static uint8_t temp_harq_pid = 0;
static uint8_t temp_tpc = 0;
                      ...
                      frame_t frame)
{
  ...
  max_mcs = min(max_mcs, bler_options->max_mcs); // max_mcs: 28
  const uint8_t old_mcs = min(bler_stats->mcs, max_mcs); // 9
  if (diff < BLER_UPDATE_FRAME) // 10 < 10
                  ...
                  NR_UE_UL_BWP_t *ul_bwp)
{
  ...
  // mwnl or
  // Get the RNTI of the UE connected to the gNB.
  if(first_rnti == 0)
    first_rnti = pusch_pdu->rnti;
  if(first_rnti != pusch_pdu->rnti)
    second_rnti = pusch_pdu->rnti;
  // ======================================

  // mwnl or
  // In the DSI format, FDA is used for group(multiple UEs). This area calculates the FDA for the group(multiple UEs).
  int n_UEs = 2;
  dci_pdu_rel15->frequency_domain_assignment.val = 
  PRBalloc_to_locationandbandwidth0(pusch_pdu->rb_size, pusch_pdu->rb_start, ul_bwp->BWPSize);
  dci_pdu_rel15->frequency_domain_assignment.val = dci_pdu_rel15->frequency_domain_assignment.val + ((n_UEs-1)*pusch_pdu->rb_size*ul_bwp->BWPSize);
  // ======================================
  ...
  // mwnl or
  // Set the DSI format info.
  dci_pdu_rel15->fiveQI = 80;
  dci_pdu_rel15->DSI_dedicated = 0;

  srand(time(0));

  // In this area, you can identify the UE for which the DSI is transmitted.
  if(first_rnti == pusch_pdu->rnti)
  {
    // Simulating real traffic BSR.
    dci_pdu_rel15->buffer_size = rand() % 32;
    // dci_pdu_rel15->buffer_size = 7; //7
    // srand(time(0)); 
    
    dci_pdu_rel15->DSI_dedicated |= (temp_buffer_size << 9); 
    dci_pdu_rel15->DSI_dedicated |= (temp_ndi << 8); 
    dci_pdu_rel15->DSI_dedicated |= (temp_rv << 6); 
    dci_pdu_rel15->DSI_dedicated |= (temp_harq_pid << 2); 
    dci_pdu_rel15->DSI_dedicated |= (temp_tpc);

    DSI_flag = 0;
    temp_buffer_size = dci_pdu_rel15->buffer_size;
    temp_ndi = dci_pdu_rel15->ndi;
    temp_rv = dci_pdu_rel15->rv;
    temp_harq_pid = dci_pdu_rel15->harq_pid;
    temp_tpc = dci_pdu_rel15->tpc;
  }
  else
  {
    // Simulating real traffic BSR.
    dci_pdu_rel15->buffer_size = rand() % 32;
    // dci_pdu_rel15->buffer_size = 11; // 11

    dci_pdu_rel15->DSI_dedicated |= (temp_buffer_size << 9);
    dci_pdu_rel15->DSI_dedicated |= (temp_ndi << 8);
    dci_pdu_rel15->DSI_dedicated |= (temp_rv << 6);
    dci_pdu_rel15->DSI_dedicated |= (temp_harq_pid << 2);
    dci_pdu_rel15->DSI_dedicated |= (temp_tpc << 2);
    ...
    switch (rnti_type) {
      case TYPE_C_RNTI_:
        ...
        *dci_pdu |= ((uint64_t)dci_pdu_rel15->mcs & 0x1f) << (dci_size - pos);
        ...
        // mwnl or
        // Convert the DSI format to a bit string.
        // 5QI 7bit
        pos += 7;
        *dci_pdu |= ((uint64_t)dci_pdu_rel15->fiveQI & ((1 << 7) - 1)) << (dci_size - pos);
        // BSR 5bit
        pos += 5;
        *dci_pdu |= ((uint64_t)dci_pdu_rel15->buffer_size & ((1 << 5) - 1)) << (dci_size - pos);
        ...
        *dci_pdu |= ((uint64_t)dci_pdu_rel15->tpc & 0x3) << (dci_size - pos);
        ...
        // DSI dedicated
        pos += 14;
        *dci_pdu |= ((uint64_t)dci_pdu_rel15->DSI_dedicated & ((1 << 14) - 1)) << (dci_size - pos);
...
// #ifdef DEBUG_DCI
        ...
        if (DSI_flag == 0)
        {
          ...
          LOG_I(NR_MAC,"                [UE 2]                \n");
          LOG_I(NR_MAC,"RNTI = %i\n", second_rnti);
          LOG_I(NR_MAC,"Buffer Status Report = %i\n", ((dci_pdu_rel15->DSI_dedicated)>> 9) & 31);
