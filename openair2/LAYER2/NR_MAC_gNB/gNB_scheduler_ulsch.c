  ...
  while (!done && pdu_len > 0){
    ...
    switch(rx_lcid){
      ...
      case UL_SCH_LCID_S_TRUNCATED_BSR:
        ...
        LOG_D(NR_MAC,
              "[BSR] SHORT BSR at %4d.%2d, LCG ID %d, BS Index %d, BS value < %d, est buf %d\n",
              frameP,
              slot,
              bsr_s->LcgID,
              bsr_s->Buffer_size,
                       ...
                       const nfapi_nr_crc_t *crc_pdu)
{
  ...
  if (!crc_pdu->tb_crc_status) {
    ...
    harq->round = 0;
    LOG_D(NR_MAC,
          "Ulharq id %d crc passed for RNTI %04x,ndi is %d \n",
          harq_pid,
          crc_pdu->rnti,
          harq->ndi
          );
...
static bool nr_UE_is_to_be_scheduled(const NR_ServingCellConfigCommon_t *scc, int CC_id, 
                                     NR_UE_info_t* UE, frame_t frame, sub_frame_t slot, 
                                     uint32_t ulsch_max_frame_inactivity)
{
  ...
  // mwnl
  // printf("has_data:%d, estimated_ul_buffer:%d, sched_ul_bytes:%d \n",has_data,sched_ctrl->estimated_ul_buffer,sched_ctrl->sched_ul_bytes);
...
static void pf_ul(module_id_t module_id,frame_t frame,sub_frame_t slot,
                  NR_UE_info_t *UE_list[],int max_num_ue,int n_rb_sched,
                  uint16_t *rballoc_mask)
{
  ...
  UE_iterator(UE_list, UE) {
    ...
    if (bo->harq_round_max == 1)
      ...
      sched_pusch->mcs = get_mcs_from_bler(bo, stats, &sched_ctrl->ul_bler_stats, max_mcs, frame); //this one
